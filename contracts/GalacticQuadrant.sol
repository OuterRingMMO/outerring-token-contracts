...
contract LGEWhitelisted is Context {
    ...
    event WhitelisterTransferred(
        address indexed previousWhitelister,
    ...
    function transferWhitelister(address newWhitelister)
        external
        onlyWhitelister
    {
    ...
    function createLGEWhitelist(
        ...
        if (durations.length > 0) {
            ...
            for (uint256 i = 0; i < durations.length; i++) {
                WhitelistRound storage whitelistRound = _lgeWhitelistRounds
                    .push();
    ...
    function _applyLGEWhitelist(
        ...
        if (
            _lgeTimestamp == 0 &&
            sender != _lgePairAddress &&
            recipient == _lgePairAddress &&
            amount > 0
        ...
        if (sender == _lgePairAddress && recipient != _lgePairAddress) {
            ...
            if (wlRoundNumber > 0) {
                WhitelistRound storage wlRound = _lgeWhitelistRounds[
                    wlRoundNumber - 1
                ];

                require(
                    wlRound.addresses[recipient],
                    "LGE - Buyer is not whitelisted"
                );
                ...
                require(
                    ...
                    "LGE - Amount exceeds whitelist maximum"
                );
                wlRound.purchased[recipient] =
                    wlRound.purchased[recipient] +
                    amount;
...
abstract contract BEP20 is Context, IBEP20, LGEWhitelisted {
...
contract GalacticQuadrant is BEP20, AccessControl {

    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");

    uint256 private immutable _cap;

    constructor(uint256 cap_) BEP20("Galactic Quadrant", "GQ") {
        _cap = cap_;
    }
    ...
    {
        require(super.totalSupply() + amount <= _cap, "BEP20Capped: cap exceeded");
