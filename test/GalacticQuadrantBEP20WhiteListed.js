...
const deployWithoutLGE = deployments.createFixture(async ({ deployments, ethers }, options) => {
  ...
  const pad = await ethers.getContract("GalacticQuadrant");
...
describe("BSCPad Test Suite", () => {
  ...
  describe("Deployment", () => {
    ...
    it("name should be GalacticQuadrant", async () => {
      const { pad } = await deployWithLGE();
      expect(await pad.name()).to.be.equal("Galactic Quadrant");
    });

    it("symbol should be GQ", async () => {
      const { pad } = await deployWithLGE();
      expect(await pad.symbol()).to.be.equal("GQ");
    });

    it("deployer should have the admin role", async () => {
      const { pad, deployer } = await deployWithLGE();
      const DEFAULT_ADMIN_ROLE = await pad.DEFAULT_ADMIN_ROLE();
      expect(await pad.hasRole(DEFAULT_ADMIN_ROLE, deployer.address)).true;
    });
    ...
    it("Max cap should be 10 US billion tokens", async () => {
      const { pad } = await deployWithLGE();
      expect(await pad.cap()).to.be.equal(tokens("10000000000"));
    });
  ...
  describe("Access control", () => {
    ...
    it("only the admin can grant role to another address", async () => {
      const { pad, holder5 } = await deployWithLGE();
      const MINTER_ROLE = await pad.MINTER_ROLE();
      await expect(pad.connect(holder5).grantRole(MINTER_ROLE, holder5.address)).to.be.reverted;
      expect(await pad.hasRole(MINTER_ROLE, holder5.address)).false;
    });

    it("only the admin can revoke role to another address", async () => {
      const { pad, deployer, holder5 } = await deployWithLGE();
      const MINTER_ROLE = await pad.MINTER_ROLE();
      await pad.connect(deployer).grantRole(MINTER_ROLE, holder5.address);
      expect(await pad.hasRole(MINTER_ROLE, holder5.address)).true;
      await expect(pad.connect(holder5).revokeRole(MINTER_ROLE, holder5.address)).to.be.reverted;
      expect(await pad.hasRole(MINTER_ROLE, holder5.address)).true;
    });
