...
async function main() {
    ...
    const SCK = await hre.deployments.get('SCK');
    const sck = await ethers.getContractAt('Resource', SCK.address);
    ...
    const payload = sck.interface.encodeFunctionData("grantRole", [MINTER_ROLE, newMinterAddress]);
    const ether = parseEther("0.0")
    let tx = await multiSig.submitTransaction(sck.address, ether, payload);
    console.log(tx);
    sleep(5000);
