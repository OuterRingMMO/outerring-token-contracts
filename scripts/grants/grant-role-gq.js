...
async function main() {
    ...
    const gq = await ethers.getContractAt('GalacticQuadrant', GQ.address);
    ...
    const payload = gq.interface.encodeFunctionData("grantRole", [MINTER_ROLE, newMinterAddress]);
    const ether = parseEther("0.0")
    let tx = await multiSig.submitTransaction(gq.address, ether, payload);
    console.log(tx);
    sleep(5000);
